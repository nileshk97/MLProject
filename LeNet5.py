...
def save_model(model,acc):
    ...
    torch.save(state,'Lenet5.pkl')

def accuracy(y_pred,y):
    pred = torch.max(y_pred,1)[1]
    correct = (pred == y).long().sum() 
    acc = (correct.data/ len(y))
    return acc
...
for epoch in range(epoch_no):
    for step,(batch_x,batch_y) in enumerate(train_loader):
        ...
        if step % 20 == 0:
            print('Epoch : %d   ||  step = %d   || loss = %f   || train_acc = %f' % (epoch,step,loss.data,train_acc))
        ...
        if step % 100 == 0:
            ...
            for test_x,test_y in test_loader:
                ...
                test_loss += loss_fn_test(test_y_pred,test_y).data
